...
const sendEmail = async (options) => {
  ...
  // ── [2] Gmail API via OAuth2 (HTTPS Port 443) - Bypasses SMTP ports ──
  if (process.env.GMAIL_CLIENT_ID && process.env.GMAIL_CLIENT_SECRET && process.env.GMAIL_REFRESH_TOKEN) {
    console.log('[sendEmail] Using true Gmail REST API (HTTPS)');
    ...
    console.log('[sendEmail] Gmail API delivered:', res.data.id);
    return;
  }
